...
interface ArticleModalProps {
  ...
  /**
   * Full article list — used to resolve relatedArticleIds.
   * Accepts Article[] or ArticleWithCoverage[] (structural typing).
   */
  allArticles: Article[];
}
...
export const ArticleModal: React.FC<ArticleModalProps> = ({ article, onClose, allArticles }) => {
  ...
  // O(1) article lookup map — built once per modal open.
// useMemo prevents rebuilding on every render.
const articleMap = useMemo(
  () => new Map<number, Article>(allArticles.map((a) => [a.id, a])),
  [allArticles]
);
...
//
// Priority 1:
//   Use semantic relatedArticleIds generated by dedupEngine.
//
// Fallback:
//   Use old category/tag heuristic for plain Article objects.
const related = useMemo((): Article[] => {
  // Enriched path — semantic related IDs
  if (
    "relatedArticleIds" in article &&
...
}, [article, allArticles, articleMap]);
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "4px 28px 32px" }}>
          ...
          {/* Related articles — semantic backend-driven linking */}
          {related.length > 0 && (
    ...
    >
      {related.map((r) => (
        ...
        <button
          ...
          aria-label={`Open related article: ${r.title}`}
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.background = DS.bg3;
